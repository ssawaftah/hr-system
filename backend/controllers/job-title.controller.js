...
const groupPermissionDefinitions = () => PERMISSION_DEFINITIONS.reduce((groups, permission) => {
...
const requireAnyPermission = async (req, res, permissions = []) => {
  const access = await getAccess(req);
  const allowed = access.roles.includes("admin") || permissions.some((permission) => hasPermissionValue(access, permission));
  if (!allowed) {
...
const logPermissionChange = async ({ req, targetType, targetId, changeType, oldValue = null, newValue = null, reason = null }) => {
  try {
    ...
    console.error("PERMISSION LOG ERROR:", error.message);
  }
};
...
const getPermissionDefinitions = async (req, res) => {
  try {
    ...
    const access = await requireAnyPermission(req, res, ["permissions.view", "permissions.manage"]);
    if (!access) return;
    res.status(200).json({ permissions: PERMISSION_DEFINITIONS, groups: groupPermissionDefinitions() });
  } catch (error) {
    res.status(500).json({ error: error.message || "حدث خطأ أثناء جلب الصلاحيات" });
  }
};
...
const getJobTitles = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["job_titles.view", "permissions.view", "permissions.manage"]);
    if (!access) return;
...
const createJobTitle = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["job_titles.create", "job_titles.manage_permissions", "permissions.manage"]);
    if (!access) return;
...
const updateJobTitle = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["job_titles.update", "job_titles.manage_permissions", "permissions.manage"]);
    if (!access) return;
    ...
    if (duplicate.rows.length) return res.status(400).json({ error: "يوجد مسمى وظيفي بنفس الاسم" });
    const result = await pool.query(
      `UPDATE job_titles SET name=$1, code=$2, description=$3, default_permissions=$4::text[], status=$5, updated_at=CURRENT_TIMESTAMP WHERE id=$6 RETURNING *`,
...
const disableJobTitle = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["job_titles.disable", "permissions.manage"]);
    if (!access) return;
    const id = Number(req.params.id);
    const old = await pool.query(`SELECT * FROM job_titles WHERE id=$1`, [id]);
    if (!old.rows.length) return res.status(404).json({ error: "المسمى الوظيفي غير موجود" });
...
const deleteJobTitle = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["job_titles.delete", "permissions.manage"]);
    if (!access) return;
    const id = Number(req.params.id);
    const old = await pool.query(`SELECT * FROM job_titles WHERE id=$1`, [id]);
    if (!old.rows.length) return res.status(404).json({ error: "المسمى الوظيفي غير موجود" });
...
const getEmployeePermissions = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["employee_permissions.view", "permissions.view", "permissions.manage"]);
    if (!access) return;
...
const updateEmployeePermissions = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["employee_permissions.manage", "permissions.manage"]);
    if (!access) return;
    ...
    await pool.query(`UPDATE users SET roles=$1::text[], role=$2, permissions=$3::text[], direct_denied_permissions=$4::text[] WHERE id=$5`, [roles, roles[0] || "employee", directPermissions, deniedPermissions, oldSummary.user_id]);
...
const getPermissionLogs = async (req, res) => {
  try {
    await ensurePermissionSchema();
    const access = await requireAnyPermission(req, res, ["permissions.view_logs", "permissions.manage"]);
    if (!access) return;
    const result = await pool.query(`SELECT * FROM permission_audit_logs ORDER BY id DESC LIMIT 100`);
    res.status(200).json({ logs: result.rows });
  } catch (error) {
    res.status(500).json({ error: error.message || "حدث خطأ أثناء جلب سجل الصلاحيات" });
  }
};
