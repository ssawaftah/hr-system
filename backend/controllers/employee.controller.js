...
const ensureEmployeeSchema = async (client = pool) => {
  ...
  await client.query(`
    UPDATE employees
    SET employee_number = COALESCE(employee_number, NULLIF(national_id, ''), id::text)
    WHERE employee_number IS NULL OR employee_number = ''
  `);

  await client.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS employees_employee_number_unique_idx
...
const writeAuditLog = async (client, req, action, targetId, oldValue = null, newValue = null) => {
  await client.query(
    `
    INSERT INTO audit_logs (actor_user_id, target_type, target_id, action, old_value, new_value)
...
const normalizeDepartments = (body) => {
  ...
  const cleaned = departments
    .map((item) => ({
      ...
      is_primary: normalizeBoolean(item.is_primary),
      role_in_department: item.role_in_department || null,
...
const validateEmployeePayload = async (client, body, currentId = null) => {
  ...
  const duplicate = await client.query(
    `SELECT id FROM employees WHERE employee_number = $1 AND ($2::INTEGER IS NULL OR id <> $2::INTEGER) LIMIT 1`,
...
const enrichEmployees = async (rows) => {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      additional_departments: memberships.filter((item) => !item.is_primary),
    };
  });
};
...
const createEmployee = async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      ...
      (employee_number, full_name, national_id, phone, email, address, job_title, direct_manager_id, department_id, hire_date, employment_type, basic_salary, social_security_enabled, social_security_rate, status, is_active, updated_at)
